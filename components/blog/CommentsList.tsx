  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {comments.map(comment => {
        ...
        const date = new Date(comment.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
        return (
          <div key={comment.id} style={{ background: '#F7F8FA', borderRadius: 12, padding: '1.2rem 1.4rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#00447B', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: 14, flexShrink: 0 }}>
                {name.charAt(0).toUpperCase()}
              </div>
              <div>
                <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 600, fontSize: 14, color: '#00447B' }}>{name}</div>
                <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, color: '#9ca3af' }}>{date}</div>
              </div>
            </div>
