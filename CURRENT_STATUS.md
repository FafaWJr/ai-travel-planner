...
**Last updated:** 12 May 2026 (Phase 7 ship + security hardening)
**Maintainer:** Wilson
...
**Last shipped release:** Luna Memories Phase 7 — PWA layer + security hardening (12 May 2026, `88bd5523` / `e1f7491c`). Luna is now installable as a Progressive Web App on mobile: manifest, service worker (cache-first static / network-first pages), offline fallback, maskable icons, `PwaInstallBanner` component. Security hardening pass resolved five issues (C2/C3/C4/H2/H3): dropped anonymous-enumeration RLS policy, moved PDF export to private `memory-pdfs` bucket, fixed auth redirect to honour `luna_redirect_after_login` convention, added `.eq('user_id')` IDOR guard on trip queries, stripped private fields from public share responses. Also shipped Phase 6 (PDF export `12103fa6` + Stripe paywall `65a6f8f6`, 12 May) and HF-4 dual-lookup for photos/narrative routes (`998d57de`, 12 May).
...
- **Luna Memories Phases 1+2+3+4+5+6+7 shipped** (12 May 2026): `trip_memories` table + RLS, memory capture page, streaming AI narrative, shareable public link + OG image, mid-trip capture banner, photo storage foundation (`memory-photos` bucket, exif.ts, compress.ts, photos API), photo upload UI with EXIF auto-sort + day grid + lightbox (`BulkPhotoUpload`, `DayPhotoGrid`, `lib/memories/types.ts`, 18 locale keys), GPS route map (`RouteMap.tsx`, Leaflet + OpenStreetMap, day-coloured markers + polylines), photo gallery on public share page (grouped by day, responsive hero grid), `/memories` landing page + standalone memory creation (`POST /api/memories/standalone`) + AI day skeleton (`POST /api/memories/skeleton`) + dual-lookup on GET/PUT + Memories nav link, 15 new locale keys per locale + nav.memories, locale diacritics fix, branded Stories + carousel sharing cards (`/api/memories/card/[token]`, `/api/memories/card/day/[token]/[dayNumber]`) + 4-button share panel on capture page, 5 new locale keys per locale, premium PDF export via Puppeteer + @sparticuz/chromium (`/api/memories/export/pdf`), Stripe $10 one-off paywall (`/api/payments/create-session` + `/api/webhooks/stripe`), PWA layer (manifest, service worker, offline page, maskable icons, `PwaInstallBanner`), security hardening (private PDF bucket, IDOR guard, share data stripping). Phase 7 complete. Next: Phase 8 (Peecho print book) or Phase 9 (re-engagement loop). Spec at `docs/specs/memories/session_anchor.md`.
...
**Active phase:** Phase 8 (not started) — Peecho print book, OR Phase 9 (re-engagement loop). Wilson to confirm next phase.
**Phase status:** Phase 7 fully complete. Phase 8 and Phase 9 not started.
**Last completed phase:** Phase 7 — PWA layer (shipped 12 May 2026, commit `88bd5523`). Also shipped same day: Phase 6.1 (`12103fa6`), Phase 6.2 (`65a6f8f6`), HF-4 (`998d57de`), security hardening (`e1f7491c`).
