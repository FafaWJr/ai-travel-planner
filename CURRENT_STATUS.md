...
**Last updated:** 13 May 2026 (LP-3 destination autocomplete + sessionStorage auth flow on /memories)
**Maintainer:** Wilson
...
**Last shipped release:** Luna Memories LP-3 — destination autocomplete and sessionStorage auth flow on /memories (13 May 2026, `54c3a40f`). Extracts `components/DestinationInput.tsx` from `HeroStepForm` as a shared geocoding autocomplete component (optional `inputStyle?` and `iconColor?` props). `HeroStepForm` refactored to import it — zero behaviour change on /start. `/memories` landing form now uses `DestinationInput` with full geocoding autocomplete + thumbnails + keyboard-nav UX, identical to /start. Anonymous user flow replaced: form data is now saved to sessionStorage under `luna_memories_form`, `luna_redirect_after_login` is set to `/${locale}/memories`, and the user is redirected to `/auth/login`; after sign-in a `useEffect` detects the stored form data, auto-fills fields, auto-creates the standalone memory (500ms settle delay), navigates to the capture page, and clears sessionStorage immediately to prevent double-creation. Removed `isLoginError` state and `loginRequired`/`signInLink` locale keys from `memoriesLanding` namespace (EN/PT-BR/ES). Error paragraph simplified to validation/network errors only.
...
|---|---|---|---|---|
| Luna Memories LP-3: destination autocomplete + sessionStorage auth flow | Closed | `54c3a40f` | 13 May | Shared DestinationInput component extracted from HeroStepForm; /memories gets geocoding autocomplete UX identical to /start; anonymous flow uses sessionStorage + redirect instead of inline error; removed loginRequired/signInLink locale keys |
...
**Active phase:** LP-3 shipped. Phase 8 (Peecho print book) or Phase 9 (re-engagement loop) next — Wilson to confirm.
**Phase status:** Phases 1–7, LP-2 (landing redesign), and LP-3 (destination autocomplete + sessionStorage auth) fully complete. Phase 8 and Phase 9 not started.
**Last completed phase:** LP-3 — shared DestinationInput component + sessionStorage anonymous auth flow on /memories (shipped 13 May 2026, commit `54c3a40f`). Removed `loginRequired`/`signInLink` locale keys from `memoriesLanding` namespace (EN/PT-BR/ES).
...
**Phase LP-3 deliverables (all shipped):**
- LP-3 (`54c3a40f`, 13 May): `components/DestinationInput.tsx` extracted from `HeroStepForm` — shared geocoding autocomplete component with optional `inputStyle?` and `iconColor?` props; identical UX to /start (geocoding, thumbnails, keyboard navigation). `components/HeroStepForm.tsx` refactored to import the extracted component — zero behaviour change on /start. `/memories` landing form updated to use `DestinationInput` with full autocomplete. Anonymous user flow replaced: form data saved to sessionStorage under `luna_memories_form`; `luna_redirect_after_login` set to `/${locale}/memories`; redirect to `/auth/login`. After sign-in: `useEffect` detects stored form data, auto-fills fields, auto-creates the standalone memory (500ms settle delay to allow Supabase session hydration), navigates to the capture page, clears sessionStorage immediately to prevent double-creation. Removed `isLoginError` state. Removed `loginRequired` and `signInLink` keys from `memoriesLanding` namespace in EN/PT-BR/ES. Error paragraph simplified to validation/network errors only (red `#DC2626`). Phase LP-3 complete.
