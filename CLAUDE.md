...
**Last Updated:** 2026-04-21 18:48:20
**Current Branch:** main
**Last Commit:** 75cce3c2 docs: add AI-PATHS, PHASE-MODE-MATRIX, POST-R6-CHANGELOG reference docs
...
Before diving into the project code, read these three docs. They document architecture and history that are not obvious from reading the source.

- **`AI-PATHS.md`** — the two parallel AI invocation paths in the codebase (generate-path uses Anthropic tools API, chat-path uses text markers). Rules for adding new AI capabilities to either path. Read this BEFORE adding any new Luna capability.
- **`PHASE-MODE-MATRIX.md`** — comprehensive reference for how short (1-6 days), medium (7-14), and long (15+) trips differ across tool arrays, prompts, rendering, and user affordances. Read this BEFORE modifying any trip-length-mode behavior.
- **`POST-R6-CHANGELOG.md`** — durable release log from R1 onward. Every release has commit hash, deployment ID, UTC timestamp, and summary. Read this to understand why any piece of code exists in its current form.

Additional persistent docs at repo root:
- **`CONVENTIONS.md`** — style, branding, and coding conventions (no em dashes, Lucide icons only, brand colors, etc.).
- **`MULTILANG-REFERENCE.md`** — translation rules for English, Brazilian Portuguese, and Spanish.
- **`COLLABORATIVE-TRIPS-PLAN.md`** — v1.1 spec for the upcoming collaborative-editing feature (not yet shipped).

This `CLAUDE.md` file is auto-regenerated by `update-context.sh`. Any edits to this file directly will be lost on next regeneration. To add persistent content, edit the heredoc template inside the script itself.
...
```
75cce3c2 (HEAD -> main, origin/main, origin/HEAD) docs: add AI-PATHS, PHASE-MODE-MATRIX, POST-R6-CHANGELOG reference docs
