...
- **What:** When a user drags an activity card from one time slot to another, the Supabase Broadcast patch did not reach the other collaborator's browser. The emit fired but the receive handler did not process it.
- **Resolution:** Fixed by commit `e2c6cdc9` (30 April 2026, Stage 3 pre-blocker P2-3b). Dropped dead day-id gate; repositioned `sourceSlot` read to use `patch.sourceSlot` instead of the live activity slot. Confirmed resolved by Wilson, 6 May 2026.
- **Resolved:** May 2026

### TD-008: CLAUDE.md is stale (last regenerated April 12, 2026)

- **What:** `CLAUDE.md` was last generated by `scripts/update-context.sh` on 2026-04-12. Multiple releases had shipped without a context regen.
- **Resolution:** Heredoc template updated in `scripts/update-context.sh` to include `TECH_DEBT.md` reference; `CLAUDE.md` regenerated and committed on 5 May 2026 (commit `db929e68`). Context is now current.
- **Resolved:** May 2026
